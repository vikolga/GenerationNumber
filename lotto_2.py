...
radio_1 = Radiobutton(window, text = '49', variable = interval, value = 49)
radio_2 = Radiobutton(window, text = '72', variable = interval, value = 72)
radio_3 = Radiobutton(window, text = '99', variable = interval, value = 99)
getBtn = Button(window)
resBtn = Button(window)


# добавляем виджеты на экран
imgLbl.grid(row = 1, column = 1, rowspan = 3)
...
getBtn.grid(row = 3, column = 2, columnspan = 4)
resBtn.grid(row = 3, column = 5, columnspan = 4)
radio_1.grid(row = 2, column = 2)
radio_2.grid(row = 2, column = 4)
radio_3.grid(row = 2, column = 6)
...
def pick():
    nums = sample(range(1, interval.get()), 6)
